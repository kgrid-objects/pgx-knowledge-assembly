...
function phenotype(inputs) {
  ...
  if (
    list[alleles[0]] !== 'Unknown Function' &&
    list[alleles[1]] !== 'Unknown Function' &&
    list[alleles[0]] !== 'Uncertain Function' &&
    list[alleles[1]] !== 'Uncertain Function'
  ) {
    alleles.forEach((e) => {
      switch (list[e]) {
        case 'Normal function':
          ...
          break;
        case 'No function':
        case 'No Function': // In case of inconsistent capitalization
          count += 1;
          break;
      }
    });
  ...
  if (count > 0) {
    output[gene].phenotype += ' activity';
  }
